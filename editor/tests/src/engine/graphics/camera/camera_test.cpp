...
TEST_CASE("Camera", "[camera]")
{
    ...
    SECTION("can convert from world to screen pos")
    {
        ...
        REQUIRE(camera.worldToScreenPos(10.0f, -10.0f) == Vec2(500.0f, -500.0f));
        REQUIRE(camera.worldToScreenPos(-10.0f, 10.0f) == Vec2(0, 0));
        REQUIRE(camera.worldToScreenPos(-8.0f, 8.0f) == Vec2(50.0f, -50.0f));
    }
    ...
    SECTION("can convert from world to screen pos with camera translate")
    {
        ...
        REQUIRE(camera.worldToScreenPos(15.0f, -10.0f) == Vec2(500.0f, -500.0f));
        REQUIRE(camera.worldToScreenPos(-5.0f, 10.0f) == Vec2(0, 0));
        REQUIRE(camera.worldToScreenPos(-3.0f, 8.0f) == Vec2(50.0f, -50.0f));
    }
}